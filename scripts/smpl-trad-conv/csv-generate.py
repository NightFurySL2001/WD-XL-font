...
for tc_to_sc in (True, False):
    ...
    if tc_to_sc:
        ...
        output_file = open("csv-trad-to-simp.txt", "w", encoding="utf-8")
    else:
        one_to_many = open("sc-tc-1-m.txt", "r", encoding="utf-8")
        one_to_one = open("sc-tc-1-1.txt", "r", encoding="utf-8")
        output_file = open("csv-simp-to-trad.txt", "w", encoding="utf-8")
    ...
    written=[]
    output_file.write("#START ONE-TO-MANY\n")
    for line in one_to_many:
        ...
        output_file.write(convert_uni(orig)+"->")
        out_list=[]
        for word in temp:
            out_list.append(convert_uni(word))
        output_file.write(";".join(out_list))
        #end writing
        output_file.write("\n")
        written.append(orig)

    output_file.write("#START ONE-TO-ONE\n")
    for line in one_to_one:
        ...
        if line[0] in font_char_list and line[2] in font_char_list and line[0] not in written:
            output_file.write(convert_uni(line[0])+"->"+convert_uni(line[2])+"\n")
