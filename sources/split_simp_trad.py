...
#write output file
output_file = open("opentype-"+conversion_name+"-split.fea", "w", encoding="utf-8")
    ...
    if multi: #one to many scenario
        ...
        if ";" in replace: #one to many
            ...
            for item in replace_arr:
                if item in diff_chars[keys[0]]: #if have 2 region
                    ...
                    output_arr.append(diff_chars[keys[1]][index_no_item])
                    output_arr.append(diff_chars[keys[2]][index_no_item])
                else:
            ...
            if search in diff_chars[keys[0]]:
                ...
                grouped_output.append("    sub "+ diff_chars[keys[1]][index_no_search] + " from [ " + " ".join(output_arr) + " ] ;\n")
                grouped_output.append("    sub "+ diff_chars[keys[2]][index_no_search] + " from [ " + " ".join(output_arr) + " ] ;\n")
            else: #search term no suffix (sub A from [X,Y,Z])
                grouped_output.append("    sub "+ search + " from [ " + " ".join(output_arr) + " ] ;\n")
        else: #one to one
            ...
            if replace in diff_chars[keys[0]]:
                ...
                if search in diff_chars[keys[0]]:
                    ...
                    grouped_output.append("    sub "+ diff_chars[keys[1]][index_no_search] + " from [ " + diff_chars[keys[1]][index_no_replace] + " " + diff_chars[keys[2]][index_no_replace] + " ] ;\n")
                    grouped_output.append("    sub "+ diff_chars[keys[2]][index_no_search] + " from [ " + diff_chars[keys[1]][index_no_replace] + " " + diff_chars[keys[2]][index_no_replace] + " ] ;\n")
                else: #seach dont hv different suffix (sub X from [-SC -TC]) -> one liner for both fonts
                    index_no_replace = diff_chars[keys[0]].index(replace)
                    grouped_output.append("    sub "+ search + " from [ " + diff_chars[keys[1]][index_no_replace] + " " + diff_chars[keys[2]][index_no_replace] + " ] ;\n")
                ...
                index_no_search = diff_chars[keys[0]].index(search)
                grouped_output.append("    sub "+ diff_chars[keys[1]][index_no_search] + " from [ " + replace + "  ;\n")
                grouped_output.append("    sub "+ diff_chars[keys[2]][index_no_search] + " from [ " + replace + "  ;\n")
            #if both dont have suffix (sub A from [X])
            else:
                grouped_output.append("    sub "+ search + " from [ " + replace + "  ;\n")
        ...
        if replace in diff_chars[keys[0]]:
            ...
            if search in diff_chars[keys[0]]:
                index_no_search = diff_chars[keys[0]].index(search)
                index_no_replace = diff_chars[keys[0]].index(replace)
                individual_output.append("    sub "+ diff_chars[keys[1]][index_no_search] + " by " + diff_chars[keys[1]][index_no_replace] + ";\n")
                individual_output.append("    sub "+ diff_chars[keys[2]][index_no_search] + " by " + diff_chars[keys[2]][index_no_replace] + ";\n")
            else: #seach dont hv different suffix (sub X by -SC/-TC) -> only used in one font
                index_no_replace = diff_chars[keys[0]].index(replace)
                differentiated_output_1.append("    sub "+ search + " by " + diff_chars[keys[1]][index_no_replace] + ";\n")
                differentiated_output_2.append("    sub "+ search + " by " + diff_chars[keys[2]][index_no_replace] + ";\n")
        #if replaced dont have different suffix, but search term does (sub [-SC -TC] by X) -> one liner for both fonts
        elif search in diff_chars[keys[0]]:
            index_no_search = diff_chars[keys[0]].index(search)
            individual_output.append("    sub [ "+ diff_chars[keys[1]][index_no_search] + " " + diff_chars[keys[2]][index_no_search] + " ] by " + replace + ";\n")
        #if both dont have suffix (sub A by B)
        else:
            individual_output.append("    sub "+ search + " by " + replace + ";\n")
